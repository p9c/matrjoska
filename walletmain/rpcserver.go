...
func GenerateRPCKeyPair(config *podopts.Config, writeKey bool) (tls.Certificate, error) {
...
// setting.
func OpenRPCKeyPair(config *podopts.Config) (tls.Certificate, error) {
...
func startRPCServers(cx *pod.State, walletLoader *wallet.Loader) (*walletrpc.Server, error) {
	T.Ln("startRPCServers")
	var (
		legacyServer *walletrpc.Server
	...
	if cx.Config.Username.V() == "" || cx.Config.Password.V() == "" {
		...
		legacyServer = walletrpc.NewServer(&opts, walletLoader, listeners, nil)
	}
...
func startWalletRPCServices(wallet *wallet.Wallet, legacyServer *walletrpc.Server) {
	if legacyServer != nil {

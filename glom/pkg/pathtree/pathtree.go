...
func New(w *gel.Window) (wg *Widget) {
	...
	w.Dark = binary.New(meta.Data{}, false, func(b bool) {})
		...
		map[string]l.Widget{
			"home": wg.Page(
				"home", gel.Widgets{
					...
					gel.WidgetSize{Widget: wg.Flex().Flexed(1, wg.H3("glom").Fn).Fn,
					},
				},
			),
		},
	...
	return func(gtx l.Context) l.Dimensions {
		return w.VFlex().
			...
			Flexed(
				...
				w.Inset(
					0.25,
					w.Responsive(int(w.Size.Load()), widget).Fn,
				).Fn,
			).Fn(gtx)
	}
}

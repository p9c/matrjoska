...
func nodeHandle(ifc interface{}) (e error) {
	...
	log.App = "  node"
	launchers.F.Ln("running node handler")
	...
	if serviceOpts.ServiceCommand != "" && runServiceCommand != nil {
		if e = runServiceCommand(serviceOpts.ServiceCommand); launchers.E.Chk(e) {
			return e
		}
		return nil
	}
	...
	if cx.Config.ClientTLS.True() || cx.Config.ServerTLS.True() {
		...
		if apputil.FileExists(cx.Config.RPCCert.V()) &&
			...
			if _, e = walletmain.GenerateRPCKeyPair(cx.Config, true); launchers.E.Chk(e) {
			}
		}
	}
	// if cx.Config.NodeOff.False() {
	// go func() {
	if e := node.Main(cx); launchers.E.Chk(e) {
		launchers.E.Ln("error starting node ", e)
	}
	// }()
	launchers.I.Ln("starting node")
	if cx.Config.DisableRPC.False() {
		cx.RPCServer = <-cx.NodeChan
		cx.NodeReady.Q()
		cx.Node.Store(true)
		launchers.I.Ln("node started")
	}
	// }
	cx.WaitWait()
	launchers.I.Ln("node is now fully shut down")
	cx.WaitGroup.Wait()
	<-cx.KillAll
	return nil
}
